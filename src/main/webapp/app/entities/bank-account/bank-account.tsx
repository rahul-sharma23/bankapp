  ...
  return (
    <div>
      <h2 id="bank-account-heading" data-cy="BankAccountHeading">
        <Translate contentKey="bankuiApp.bankAccount.home.title">Bank Accounts</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="bankuiApp.bankAccount.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/bank-account/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="bankuiApp.bankAccount.home.createLabel">Create new Bank Account</Translate>
          </Link>
        </div>
      </h2>
        ...
        {bankAccountList && bankAccountList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="bankuiApp.bankAccount.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th>
                  <Translate contentKey="bankuiApp.bankAccount.user">User</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="bankuiApp.bankAccount.customer">Customer</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {bankAccountList.map((bankAccount, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`bankuiApp.AccountType.${bankAccount.accountType}`} />
                  </td>
                  <td>{bankAccount.balance}</td>
                  <td>
                    {bankAccount.createdDate ? <TextFormat type="date" value={bankAccount.createdDate} format={APP_DATE_FORMAT} /> : null}
                  </td>
                  <td>{bankAccount.user ? bankAccount.user.login : ''}</td>
                  <td>{bankAccount.customer ? <Link to={`/customer/${bankAccount.customer.id}`}>{bankAccount.customer.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="bankuiApp.bankAccount.home.notFound">No Bank Accounts found</Translate>
            </div>
          )
        )}
      </div>
